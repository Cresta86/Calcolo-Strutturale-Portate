...
Il workflow incluso in `.github/workflows/pages.yml` pubblica automaticamente la cartella `dist` su GitHub Pages a ogni push sul branch `main`. È presente anche un `index.html` nella radice, quindi il progetto funziona se GitHub Pages è configurato su **Deploy from a branch → `/ (root)`**: in quel caso inoltra automaticamente alla cartella `dist`.
